"""
============================================================================
IRCNN TRAINING SCRIPT - HYBRID APPROACH
============================================================================
Train the Inverted Residual CNN (IRCNN) for feature extraction

ARCHITECTURE:
- 94 layers total
- 7 inverted residual block groups
- 5.3M parameters
- Outputs 1282-dimensional features
...
def setup_gpu(env_config):
    ...
    if gpus:
        try:
            ...
            # Set memory limit for local environment (3.5GB for 4GB VRAM)
...
def plot_training_history(history, save_path):
    ...
    axes[0].set_title('IRCNN - Model Accuracy')
    ...
    axes[1].set_title('IRCNN - Model Loss')
...
def train_ircnn(env_config=None, epochs=100):
    """
    Train IRCNN with hybrid batch size support
    ...
    """
    print("\n" + "=" * 70)
    print("IRCNN TRAINING - HYBRID APPROACH")
    print("=" * 70)
    ...
    # Build IRCNN model
    print("\nBuilding IRCNN Model:")
    print("-" * 50)
    ircnn = InvertedResidualCNN()
    model = ircnn.build_model(include_top=True, num_classes=len(class_names))
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(checkpoint_dir, 'ircnn_best.h5'),
        ...
        keras.callbacks.TensorBoard(
            log_dir=os.path.join(env_config['output_path'], 'logs', 'ircnn'),
            histogram_freq=1
        )
    ]
    ...
    # Save final model
    final_model_path = os.path.join(checkpoint_dir, 'ircnn_final.h5')
    model.save(final_model_path)
    print(f"\n✓ Final model saved: {final_model_path}")
    
    # Save training history plot
    plot_path = os.path.join(env_config['output_path'], 'results', 'ircnn_training.png')
...
def extract_features(model, images):
    """
    Extract 1282-dimensional features from trained IRCNN
    
    This removes the classification head and uses only the feature extractor
    
    Args:
        model: Trained IRCNN model
        images: Input images
    
    Returns:
        numpy array: Extracted features (N x 1282)
    """
    print("\nExtracting IRCNN features...")
...
if __name__ == '__main__':
    ...
    # Train IRCNN
    model, history, env_config = train_ircnn(epochs=100)
    
    print("✅ IRCNN training complete!")
    print(f"Model saved in: {env_config['output_path']}/saved_models/")
    print("\nNext step: Train SACNN")
    print("Run: python training/train_sacnn.py")
